...
//        login(activity);
    ...
    private void login(Context context) {
        ...
        dialog.show();
        Api.getService(Tags.base_url).login(phone_code.replace("+", "") + phone).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).unsubscribeOn(Schedulers.io()).subscribe(new SingleObserver<Response<UserModel>>() {
            @Override
            public void onSubscribe(@NonNull Disposable d) {
                disposable.add(d);
            }
